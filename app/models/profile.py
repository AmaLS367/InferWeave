...
class HardwareRequirements(BaseModel):
    ...
    min_ram_gb: float = Field(default=16.0, ge=1.0, description="Minimum system RAM in gigabytes")
    min_cuda_version: str | None = Field(default="12.1", description="Minimum CUDA version")


class HealthcheckConfig(BaseModel):
    """Parameters for monitoring endpoint readiness."""
    path: str = Field(default="/health", description="HTTP endpoint path for readiness probe")
    port: int = Field(default=8000, description="Exposed service port")
    initial_delay_seconds: int = Field(default=10, description="Delay before first probe check")
    timeout_seconds: int = Field(default=300, description="Total timeout waiting for model readiness")
    probe_interval_seconds: int = Field(default=3, description="Interval between subsequent health checks")
...
class ModelProfile(BaseModel):
    ...
    workload_type: WorkloadType = Field(..., description="Category of the model workload")
    default_runtime: str = Field(..., description="Identifier of the default runtime template, e.g. 'vllm'")
